...
export default function ChatInterface({ docId, filename }: ChatInterfaceProps) {
  ...
  // Use a ref for the Web Speech API recognizer
  const recognitionRef = useRef<SpeechRecognition | null>(null);
  ...
  useEffect(() => {
    ...
    if (!SR) {
      // No support: leave recognitionRef null; the UI will alert if used
      return;
    }
    ...
    rec.onerror = (event: any) => {
      ...
      if (code === 'network') {
        ...
        msg = 'Microphone access denied. Please allow mic permissions and try again.';
      } else if (code === 'no-speech') {
        msg = 'No speech detected. Please try speaking again.';
      } else if (code === 'aborted') {
        // User or app stopped it—don’t alert in this case
        return;
      }

      // Log for debugging and inform user
  ...
  const handleVoiceToggle = () => {
    ...
    if (isListening) {
      ...
      try {
        ...
        alert('Unable to start speech recognition. Try again or check browser permissions.');
        setIsListening(false);
      }
    }
  };
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch('/api/ask', {
        ...
        // NOTE: If your API expects "message" instead of "query", change the key below.
        body: JSON.stringify({ docId, query: userMessage.content }),
      });
      ...
      const assistantMessage: ChatMessage = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        // NOTE: If your API returns { answer }, change data.response → data.answer.
        content: data.response,
        timestamp: new Date(),
      };
